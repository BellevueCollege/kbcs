...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		/*
		case 'time':
			echo '<input type="text" class="time ui-timepicker-input" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . $meta . '" size="10" autocomplete="off" />
					<br />' . $desc;
		break;
		*/
		// date
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( in_array( true, array( 
//				meta_box_find_field_type( 'time', $this->fields ), 
	 ...
	 */
	function admin_head() {
		if ( in_array( get_post_type(), $this->page ) && ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) /* || meta_box_find_field_type( 'time', $this->fields)*/ ) ) {
